function loco() {
    ...
    // For beta version, use different event listener syntax
    if (locoScroll.on) {
        // Old API (v3.x)
        locoScroll.on("scroll", ScrollTrigger.update);
    } else if (locoScroll.scrollerProxy) {
        // Beta API - different approach
    ...
    ScrollTrigger.addEventListener("refresh", () => {
        if (locoScroll.update) locoScroll.update();
    });
...
function setupVideoPlayers() {
    ...
    setTimeout(() => {
        ['1', '2'].forEach(id => {
            ...
            const overlay = document.getElementById(`playOverlay${id}`);

            console.log(`Video${id}:`, video, `Overlay${id}:`, overlay);

            if (!video || !overlay) return console.error(`Video${id} or Overlay${id} not found`);
            ...
            overlay.addEventListener('click', e => {
                ...
                }).catch(err => console.error(`Error playing video ${id}:`, err));
            });
